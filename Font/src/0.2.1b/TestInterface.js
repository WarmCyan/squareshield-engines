...
function startup()
{
	...
	sqrShieldMsg.blockSize = [8,8];
	sqrShieldMsg.blockMargin = -2;
	sqrShieldMsg.letterSpacing = 8;
	sqrShieldMsg.useRelPos = true;
	/*sqrShieldMsg.relPosStartX = [1,3];
	sqrShieldMsg.relPosEndX = [2,3];
	sqrShieldMsg.relPosEndY = [1,1];*/
	sqrShieldMsg.setRelativePositions([2,9],[7,9],[0,1],[1,1]);
	//sqrShieldMsg.textMargin = [100,200];
	sqrShieldMsg.centerX = true;
	sqrShieldMsg.centerY = true;
	sqrShieldMsg.setTextString("  SquareShield  Font Engine");
	texts.push(sqrShieldMsg);
	for (var i = 0; i < 8; i++) { sqrShieldMsg.chars[i].color = "#880000"; }
	
	var frameM = new Text(world);
	frameM.blockSize = [3,3];
	...
	frameM.setRelativePositions([0,1],[1,1],[24,25],[1,1]);
	frameM.setTextString("1234567890");
	texts.push(frameM);
	
	var timeM = new Text(world, [4,4], 4, true);
...
function animate()
{
	if (GAME_PLAYING)
	{
		...
		{ 
			...
			//texts[1].setTextString(String(fps));
			//log(fps);
			recordFPS = false; 
		}
