...
{
	//textH
	this.CHAR_MATRIX_HEIGHT = 7; //this needs to be constant for all characters in a single text
	...
	this.calculateTextChars = function()
	{
		...
		//TODO: add centering
		if (this.centerX) { evalX = this.absPosStartX + this.textMargin[0] + (allowedWidth - lineWidths[0]) / 2; }
		else { evalX = this.absPosStartX + this.textMargin[0]; }
		
		if (this.centerY) { evalY = this.absPosStartY + this.textMargin[1] + (allowedHeight - this.findTextHeight(lines.length)) / 2; }
		else { evalY = this.absPosStartY + this.textMargin[1]; }
