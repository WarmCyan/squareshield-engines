...
function physEvent_collision(objA, objB, keOn1, keOn2)
{
	...
	if (DAMAGE_ENABLED)
	{
		...
		{ 
			...
			objA.color = "#000000";
			if (indexOfClickedObject != -1 && objA.name == objects[indexOfClickedObject].name) { indexOfClickedObject = -1; mouseDown = false; }
			removeA = true;
		}
		...
		{ 
			...
			objB.color = "#000000";
			if (indexOfClickedObject != -1 && objB.name == objects[indexOfClickedObject].name) { indexOfClickedObject = -1; mouseDown = false; }
			removeB = true;
		}

		//remove objects as necessary
		if (removeA) { physEngine.removeEntity(objA); }
		if (removeB) { physEngine.removeEntity(objB); }
	}
}
