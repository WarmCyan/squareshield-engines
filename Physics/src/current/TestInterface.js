...
function startup()
{
	...
	//physEngine.setEntities(objects);
	...
	physEngine.addEntity(myTestObject);
	physEngine.addEntity(myOtherObject);
	physEngine.addEntity(gravityObject);
	physEngine.addEntity(collidingObject);
...
function updateObjectLog()
{
	...
	if (indexOfClickedObject != -1) { consoleText += "\nSelected object: " + physEngine.getEntities()[indexOfClickedObject].name + " (offset - x: " + parseInt(objectOffsetX) + " y: " + parseInt(objectOffsetY) + ")"; }

	if (DEBUG_ADVANCED)
	{
		for (var i = 0; i < physEngine.getEntities().length; i++)
		{
			if (physEngine.getEntities()[i].name.indexOf("Wall") > -1) { continue; }
			consoleText += "\n\nObject: " + physEngine.getEntities()[i].name;
			consoleText += "\nID: " + physEngine.getEntities()[i].physics.id;
			consoleText += "\nx: " + physEngine.getEntities()[i].physics.px;
			consoleText += "\ny: " + physEngine.getEntities()[i].physics.py;
			consoleText += "\nvx: " + physEngine.getEntities()[i].physics.vx;
			consoleText += "\nvy: " + physEngine.getEntities()[i].physics.vy;
			consoleText += "\nax: " + physEngine.getEntities()[i].physics.ax;
			consoleText += "\nay: " + physEngine.getEntities()[i].physics.ay;
			consoleText += "\nHealth: " + physEngine.getEntities()[i].health;
		}
	}
...
function spiceUp()
{
	...
	for (var i = 0; i < physEngine.getEntities().length; i++)
	{
		if (!physEngine.getEntities()[i].physics.movable) { continue; }
		var randVX = (Math.random() * 2) - 1;
		var randVY = (Math.random() * 2) - 1;

		this.physEngine.getEntities()[i].physics.vx = randVX;
		this.physEngine.getEntities()[i].physics.vy = randVY;
	}
}
function addNewGravBox()
{
	...
	//objects.push(newBox);
	physEngine.addEntity(newBox);
	log("There are now " + (physEngine.getEntities().length - 4) + " objects in the world.");
}
function addNewBox()
{
...
function clickedObjectCheck()
{
	for (var i = 0; i < physEngine.getEntities().length; i++)
	{
		var obj = physEngine.getEntities()[i];
...
function handleMouseUp(event)
{
	...
	if (indexOfClickedObject != -1)
	{
		...
		physEngine.getEntities()[indexOfClickedObject].physics.movable = true;
	}
...
function updateMouseVelocities(posX, posY, dt)
{
	...
	if (indexOfClickedObject != -1)
	{
		var obj = physEngine.getEntities()[indexOfClickedObject];
...
function physEvent_collision(objA, objB, keOn1, keOn2)
{
	...
	if (!(keOn1 < .001)) { log("Kinetic energy transfered to " + objA.name + ": " + keOn1 + " (taking " + damage1 + " damage)"); }
	if (!(keOn2 < .001)) { log("Kinetic energy transfered to " + objB.name + ": " + keOn2 + " (taking " + damage2 + " damage)"); }
	...
	if (DAMAGE_ENABLED)
	{
		...
		if (damage1 > 1 && objA.damagable) { objA.health -= damage1; }
		if (damage2 > 1 && objB.damagable) { objB.health -= damage2; }

		//using these flags so that actual removal is done AFTER checking BOTH of them (otherwise the assoc 'objects' array in this will have lost an object, and so if the last object is added, then you have an undefined if you try to check it, and it won't have a name, etc.)
		var removeA = false;
		var removeB = false;
		...
		{ 
			...
			objA.color = "#000000";
			if (indexOfClickedObject != -1 && objA.physics.id == physEngine.getEntities()[indexOfClickedObject].physics.id) { log("FOUND IT"); indexOfClickedObject = -1; mouseDown = false; }
			removeA = true;
		}
		...
		{ 
			...
			objB.color = "#000000";
			if (indexOfClickedObject != -1 && objB.physics.id == physEngine.getEntities()[indexOfClickedObject].physics.id) { log("FOUND IT"); indexOfClickedObject = -1; mouseDown = false; }
			removeB = true;
		}
		...
		if (removeA) 
		{ 
			var removedID = physEngine.removeEntity(objA); 
			if (removedID < indexOfClickedObject) { indexOfClickedObject--; }
		}
		if (removeB) 
		{ 
			var removedID = physEngine.removeEntity(objB); 
			if (removedID < indexOfClickedObject) { indexOfClickedObject--; }
		}
	}
}
