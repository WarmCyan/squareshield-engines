	...
	function resolveCollision(manifold)
	{
		...
		if (typeof physEvent_collision == "function")
		{
			...
			{ 
				...
				if ((manifold.A.vy < 0 && manifold.B.vy > 0) || (manifold.A.vy > 0 && manifold.B.vy < 0)) { totalCollisionVelocity = vel1 + vel2; console.log("ADDING Y VELOCITIES");}
				//otherwise use the difference
				else { totalCollisionVelocity = Math.abs(vel1 - vel2); console.log("SUBTRACTING Y VELOCITIES");}
			}
			else //collision normal is along x axis
			{
				...
				//if velocities have opposite signs, going against each other, so add velocities
				if ((manifold.A.vx < 0 && manifold.B.vx > 0) || (manifold.A.vx > 0 && manifold.B.vx < 0)) { totalCollisionVelocity = vel1 + vel2; console.log("ADDING X VELOCITIES");}
				//otherwise use the difference
				else { totalCollisionVelocity = Math.abs(vel1 - vel2); console.log("SUBTRACTING X VELOCITIES");}
			}
